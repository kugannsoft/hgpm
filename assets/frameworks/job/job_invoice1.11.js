...
// $('#appoDate').datepicker().datepicker("setDate", new Date());
    ...
    var return_amount=0;
    var bank_amount=0;
    // var tempEditDelete = $('#tempEditDelete').val();
...
//===========discount types===========================
  ...
  $("#workType").change(function () {
                        ...
                        success: function (json) {
                            ...
                            if (resultData) {
                                ...
                                loadProModal(
                                    ...
                                    resultData.price_stock.UnitCost,
                                    0,
    ...
    $("#addJob").click(function() {
        ...
        var no = $("#no").val();
        var qty = parseFloat($("#qty").val()).toFixed(3);
   
        if (workId == 2) {
            var Stock = parseFloat($("#stock").val());
            var priceStock = parseFloat($("#pricestock").val());
            
            if (qty > priceStock) {  
                $.notify("You can't add more than the available stock.", "warning");  
                return false;
            }
        }
        ...
        var selectedEmployeesJson = JSON.stringify(selectedEmployees);
        ...
        if(qty=='' || isNaN(qty) == true){
            ...
            $.notify("Cost Price can not be greater than selling price.", "warning");
            return false;
        }else{
            ...
            if (workId == 1 && jobdesc != '') {
                ...
                if (jobArrIndex < 0) {
                    ...
                    $("#tbl_job tbody").append("<tr estlineno='"+estLine+
                        ...
                        "' work_id='" + jobRef + 
                        "' timestamp='" + timestamp + 
                        "' totalflatPrice='" + totalflatPrice + 
                        "' selectedEmployees='" + selectedEmployeesJson + 
                        "'><td>" + k +
                        "</td><td work_id='" + workId +
                         "'>" + workTypes +
                        "</td><td>" + jobdesc +
                        "</td><td>" +qty +
                        "</td><td>" + accounting.formatNumber(sellPrice) +
                        "</td><td>" + accounting.formatNumber(prodiscount_precent) +
                        "</td><td>" + accounting.formatNumber(estprice) +
                        "</td><td>" + accounting.formatNumber(netprice) +
                        "</td><td class='text-right' style='display: none;'>" + accounting.formatNumber(totalflatPrice) +
                        "<td class='text-right'style='display: none;'>" + selectedEmployeesJson + "</td>" + 
                        "</td><td>&nbsp;<i class='glyphicon glyphicon-edit edit btn btn-info btn-xs'></i>&nbsp;<i class='remove btn btn-danger btn-xs glyphicon glyphicon-remove-circle'></i></td></tr>");
                ...
                if (workId == 2 && proCode!='') {
                    ...
                    var productArrIndex = $.inArray(proCode, proCode);
                    var itemCodeSellingPrice = proCode + '_' + sellPrice;
                  
                    var itemCodesellArrIndex = $.inArray(itemCodeSellingPrice, proCodeSellArr);
                    ...
                    if (itemCodesellArrIndex < 0) {
                        ...
                        $("#tbl_job tbody").append("<tr estlineno='"+estLine+
                            ...
                            "' insurance='" + insurance + 
                            "' work_id='" + proCode + 
                            "' timestamp='" + timestamp + "'><td>" + k + 
                            "</td><td work_id='" + workId + "'>" + workTypes + 
                            "</td><td>" + proName + 
                            "</td><td>" + accounting.formatNumber(qty) + 
                            "</td><td>" + accounting.formatNumber(sellPrice) + 
                            "</td><td>" + accounting.formatNumber(prodiscount_precent) + 
                            "</td><td>" + accounting.formatNumber(estprice) + 
                            "</td><td>" + accounting.formatNumber(netprice) + 
                        ...
                        proCodeSellArr.push(itemCodeSellingPrice);
            ...
            } else if ((workId == 5 && jobdesc.trim() != '') || (workId == 6 && jobdesc.trim() != '') || (workId == 7 && jobdesc.trim() != '') || (workId == 9 && jobdesc.trim() != '') || (workId == 10 && jobdesc.trim() != '') || (workId == 11 && jobdesc.trim() != '')) {
                //paints
                ...
                if (workId !=''  && jobdesc.trim() != ''){
                    ...
                    $.notify("Job Discription cannot to be Empty.", "warning");
                }
            }
        }
    ...
    $("#jobdesc").autocomplete({
        ...
        select: function(event, ui) {
            if (ui) {
                ...
                flatQty = ui.item.flatQty;
                $("#qty").val(1);
                $("#sellPrice").val(jobCost);
                $("#cost_price").val(jobCost);
                $("#flatQty").val(flatQty);
            } else {
                jobRef = 0;
                jobCost = 0;
...
//remove row from table
    ...
    $("#btnSave").click(function() {
        ...
        $('#tbl_job tbody tr').each(function(rowIndex, element) {
            ...
            estimatePrice.push($(this).attr('est_price'));
            costprice.push($(this).attr('cost_price'));
            estLineNo.push($(this).attr('estlineno'));
            totalflatPrice.push($(this).attr('totalflatPrice'));
            var employeeData = $(this).attr('selectedEmployees');
            if (employeeData) {
            
                try {
                    
                    var employees = JSON.parse(employeeData);
        ...
        console.log('Raw selectedEmployees  invoice data:', selectedEmployees);
        var selectedEmployeesArr = JSON.stringify(selectedEmployees);
        console.log('Stringified invoice selectedEmployees:', selectedEmployeesArr);
        ...
        if (r === true) {
            ...
            if((jobNo=='' || jobNo==0) && (cusCode==''  && regNo=='')){
                ...
                return false;
            }else if(totalNet>(cashAmount+cardAmount+chequeAmount+advance_amount+bank_amount+creditAmount+return_amount)){
                ...
                $.ajax({
                    ...
                    data: { action: action,remark:remark ,mileageout:mileageout,mileageoutUnit:mileageoutUnit,com_amount:com_amount, compayto:compayto,receiver_name:receiver_name,
                        ...
                        totalflatPrice:totalflatPrice,advancePaymentNo:advancePaymentNo,selectedEmployeesArr:selectedEmployeesArr},
                    success: function(data) {
                        ...
                        if (fb) {
                            $("#lastJob").html('');
                            $("#btnSave,#btnSaveInv,#btnSaveTemp").prop('disabled',true);
                            ...
                            $("#lastJob").html('');
                            $('#btnSave,#btnSaveInv,#btnSaveTemp').attr('disabled', false);
                        }
                    }
                });
            } else {
    ...
    $("#btnSaveTemp").click(function() {
        ...
        console.log('Raw selectedEmployees data:', selectedEmployees);
        var selectedEmployeesArr = JSON.stringify(selectedEmployees);
        console.log('Stringified selectedEmployees:', selectedEmployeesArr);
        var net_priceArr = JSON.stringify(net_price);
        var qtyArr = JSON.stringify(qty);
        var sell_priceArr = JSON.stringify(sell_price);
        var is_insArr = JSON.stringify(is_ins);
    ...
    function loadProModal(mname, mcode, msellPrice, mcostPrice, mserial, misSerial, misFree, isOP, isMP, upc, waranty, isautoSerial) {
        ...
        $("#estPrice").val(msellPrice);
        $("#cost_price").val(mcostPrice);
            ...
            success: function(data) {
                ...
                $.each(resultData.inv_dtl, function(key, value) {
                    ...
                    for (var i = 0; i < value.length; i++) {
                        ...
                        $("#tbl_est_data").append("<tr><td style='text-align:center;padding: 3px;'>" + (k) + 
                        "</td><td style='padding: 3px;' colspan='2'>" + value[i].JobDescription + 
                        "</td><td style='padding: 3px;'>" + accounting.formatMoney(value[i].JobQty) + 
                        "</td><td  style='padding: 3px;'>" + accounting.formatMoney(value[i].JobPrice) + 
                        "</td><td  style='padding: 3px;' colspan='2'>" + accounting.formatMoney(value[i].JobTotalAmount) + "</td></tr>");
                        k++;
                    }
            ...
            success: function(data) {
                ...
                $.each(resultData.inv_dtl, function(key, value) {
                    ...
                    for (var i = 0; i < value.length; i++) {
                        ...
                        "</td><td style=';padding: 3px;'>" + accounting.formatMoney(value[i].JobQty) + 
                        "</td><td  style=';padding: 3px;'>" + accounting.formatMoney(value[i].JobPrice) + 
                        "</td><td  style=';padding: 3px;' colspan='2'>" + accounting.formatMoney(value[i].JobNetAmount) + "</td></tr>");
                        k++;
                    }
...
//load edit grid data
    $("#tbl_job tbody").on('click', '.edit', function() {
        ...
        var selectedEmployees = $(this).parent().parent().attr('selectedEmployees');
        console.log("Selected Employees Attribute:", selectedEmployees); 

        var selectedEmpArray = [];
    
        if (selectedEmployees && typeof selectedEmployees === 'string') {
            try {
                // Step 1: Clean the string - remove any unwanted characters
                var cleanedSelectedEmployees = selectedEmployees.replace(/\\"/g, '"').trim();
    
                // Step 2: Remove the starting and ending quotes if they exist
                if (cleanedSelectedEmployees.startsWith('"') && cleanedSelectedEmployees.endsWith('"')) {
                    cleanedSelectedEmployees = cleanedSelectedEmployees.slice(1, -1);
                }
    
                console.log("Cleaned selectedEmployees:", cleanedSelectedEmployees);
    
                // Step 3: Now parse the cleaned string
                selectedEmpArray = JSON.parse(cleanedSelectedEmployees);
    
                if (!Array.isArray(selectedEmpArray)) {
                    selectedEmpArray = [];
                }
            } catch (e) {
                console.error("Error parsing selectedEmployees:", e, selectedEmployees);
                selectedEmpArray = [];
            }
        } else {
            console.warn("selectedEmployees is not valid:", selectedEmployees);
            selectedEmpArray = [];
        }
    
        console.log("Parsed selectedEmpArray:", selectedEmpArray);
    
        $('input[name="employees[]"]').prop('checked', false);
    
        selectedEmpArray.forEach(empID => {
            let checkbox = $('#emp_' + empID);
            console.log("Trying to check:", empID, "Checkbox found:", checkbox.length > 0);
            checkbox.prop('checked', true);
        });
    
        if ($('input[name="employees[]"]:checked').length === $('input[name="employees[]"]').length) {
        ...
        if (jobtype == 1) {
            ...
            proCodeSellArr.splice($.inArray(workid, proCodeSellArr), 1);
            $("#spartDiv").show();
            $("#jobDescDiv").hide();
            $("#flatQty").hide();
            $("#dropdownMenuButton").hide();
        ...
        // product_discount= parseFloat(((disPrecent/100)*totalPrice));
        //product_discount= parseFloat($(this).parent().parent().attr('proDiscount'));
        product_discount= parseFloat(proDiscount);
        totalProWiseDiscount += product_discount;
    ...
    function loadEstimateDatatoGrid(resultData) {
        ...
        if (resultData.est_dtl){
            ...
            for (var i = 0; i < resultData.est_dtl.length; i++) {
                ...
                $("#tbl_job tbody").append("<tr estlineno='"+resultData.est_dtl[i].estimatedtlid+
                    "' cost_price='"+resultData.est_dtl[i].EstCost+
                    "' est_price='"+resultData.est_dtl[i].EstNetAmount+
                    "' discount_type='0'  proDiscount='0' disPrecent='0'  totalPrice='"+resultData.est_dtl[i].EstTotalAmount+
                    "' isvat='"+resultData.est_dtl[i].EstIsVat+
                    "' isnbt='"+resultData.est_dtl[i].EstIsNbt+
                    "' nbtRatio='"+resultData.est_dtl[i].EstNbtRatio+
                    "' proVat='"+resultData.est_dtl[i].EstVatAmount+
                    "' proNbt='"+resultData.est_dtl[i].EstNbtAmount+
                    "'  job='" + resultData.est_dtl[i].EstJobDescription + 
                    "' jobid='" + resultData.est_dtl[i].EstJobType + 
                    "' qty='" + resultData.est_dtl[i].EstQty + 
                    "' jobOrder='" + resultData.est_dtl[i].EstJobOrder + 
                    "' netprice='" + resultData.est_dtl[i].EstNetAmount + 
                    "'  sellprice='" + resultData.est_dtl[i].EstPrice + 
                    "'  isIns='" + resultData.est_dtl[i].EstIsInsurance + 
                    "' insurance='" + resultData.est_dtl[i].EstInsurance + 
                    "' work_id='" + resultData.est_dtl[i].EstJobId + 
                    "'  timestamp='" + resultData.est_dtl[i].EstinvoiceTimestamp + 
                    "'><td>" + (i + 1) + 
                    "</td><td work_id='" + resultData.est_dtl[i].EstJobId + 
                    "'>" + resultData.est_dtl[i].jobtype_name + 
                    "</td><td>" + resultData.est_dtl[i].EstJobDescription + 
                    "</td><td>" + accounting.formatNumber(resultData.est_dtl[i].EstQty) + 
                    "</td><td>" + accounting.formatNumber(resultData.est_dtl[i].EstPrice) + 
                    "</td><td>"+accounting.formatNumber(0)+
                    "</td><td>" + accounting.formatNumber(resultData.est_dtl[i].EstNetAmount) + 
                    "</td><td>" + accounting.formatNumber(resultData.est_dtl[i].EstNetAmount) + 
                    "</td><td>&nbsp;&nbsp;<i class='glyphicon glyphicon-edit edit btn btn-info btn-xs'></i>&nbsp;<i class='remove btn btn-danger btn-xs glyphicon glyphicon-remove-circle'></i></td></tr>");
...
//load invoice data to grid
    function loadInvoiceDatatoGrid(resultData) {
        ...
        if (resultData.inv_dtl!=null || resultData.inv_dtl!=''){
            ...
            for (var i = 0; i < resultData.inv_dtl.length; i++) {
                ...
                totalProWiseDiscount += product_discount;
                // }
                $("#tbl_job tbody").append("<tr  estlineno='"+(resultData.inv_dtl[i].EstLineNo)+
                "'cost_price='"+resultData.inv_dtl[i].JobCost+
                "' est_price='"+resultData.inv_dtl[i].EstimateSellingPrice+
                "'discount_type='"+resultData.inv_dtl[i].JobDiscountType+
                "'  proDiscount='"+resultData.inv_dtl[i].JobDisValue+
                "' disPrecent='"+resultData.inv_dtl[i].JobDisPercentage+
                "'  totalPrice='"+resultData.inv_dtl[i].JobTotalAmount+
                "' isvat='"+resultData.inv_dtl[i].JobIsVat+
                "' isnbt='"+resultData.inv_dtl[i].JobIsNbt+
                "' nbtRatio='"+resultData.inv_dtl[i].JobNbtRatio+
                "' proVat='"+resultData.inv_dtl[i].JobVatAmount+
                "' proNbt='"+resultData.inv_dtl[i].JobNbtAmount+
                "'  job='" + resultData.inv_dtl[i].JobDescription + 
                "' jobid='" + resultData.inv_dtl[i].JobType + 
                "' qty='" + resultData.inv_dtl[i].JobQty + 
                "' jobOrder='" + resultData.inv_dtl[i].JobOrder + 
                "' netprice='" + resultData.inv_dtl[i].JobNetAmount + 
                "'  sellprice='" + resultData.inv_dtl[i].JobPrice + 
                "'  isIns='" + resultData.inv_dtl[i].JobPrice + 
                "' insurance='" + resultData.inv_dtl[i].JobPrice + 
                "' work_id='" + resultData.inv_dtl[i].JobCode + 
                "' totalflatPrice='" + resultData.inv_dtl[i].FlatQty + 
                "' selectedEmployees='" + JSON.stringify(resultData.inv_dtl[i].Employee) +
                "'  timestamp='" + resultData.inv_dtl[i].JobinvoiceTimestamp + "'><td>" + (i + 1) + 
                "</td><td work_id='" + resultData.inv_dtl[i].JobCode + "'>" 
                + resultData.inv_dtl[i].jobtype_name + "</td><td>" + resultData.inv_dtl[i].JobDescription + 
                "</td><td>" + accounting.formatNumber(resultData.inv_dtl[i].JobQty) + 
                "</td><td>" + accounting.formatNumber(resultData.inv_dtl[i].JobPrice) + 
                "</td><td>" + accounting.formatNumber(resultData.inv_dtl[i].JobDisPercentage) + 
                "</td><td>" + accounting.formatNumber(resultData.inv_dtl[i].EstimateSellingPrice) + 
                "</td><td>" + accounting.formatNumber(resultData.inv_dtl[i].JobNetAmount) + "</td><td>&nbsp;&nbsp;<i class='glyphicon glyphicon-edit edit btn btn-info btn-xs'></i>&nbsp;<i class='remove btn btn-danger btn-xs glyphicon glyphicon-remove-circle'></i></td></tr>");
            }
            ...
            if (resultData.inv_dtl!=null || resultData.inv_dtl!=''){
                ...
                for (var i = 0; i < resultData.inv_dtl.length; i++) {
                    ...
                    // }
                     ...
                     var actionBtns = "";
                        if (resultData.tempEditDelete.tempEditDelete == 1) {
                            
                            actionBtns = "&nbsp;&nbsp;<i class='glyphicon glyphicon-edit edit btn btn-info btn-xs'></i>" +
                    ...
                    "' disPrecent='"+resultData.inv_dtl[i].JobDisPercentage+
                    "'  totalPrice='"+resultData.inv_dtl[i].JobTotalAmount+
                    "' isvat='"+resultData.inv_dtl[i].JobIsVat+"' isnbt='"+
                    resultData.inv_dtl[i].JobIsNbt+"' nbtRatio='"+
                    resultData.inv_dtl[i].JobNbtRatio+"' proVat='"+
                    resultData.inv_dtl[i].JobVatAmount+"' proNbt='"+
                    resultData.inv_dtl[i].JobNbtAmount+"'  job='" + 
                    resultData.inv_dtl[i].JobDescription + "' jobid='" + 
                    resultData.inv_dtl[i].JobType + "' qty='" + 
                    resultData.inv_dtl[i].JobQty + "' jobOrder='" + 
                    resultData.inv_dtl[i].JobOrder + "' netprice='" + 
                    resultData.inv_dtl[i].JobNetAmount + "'  sellprice='" + 
                    resultData.inv_dtl[i].JobPrice + "'  isIns='" + resultData.inv_dtl[i].JobPrice + 
                    "' insurance='" + resultData.inv_dtl[i].JobPrice + 
                    "' work_id='" + resultData.inv_dtl[i].JobCode + 
                    "'  timestamp='" + resultData.inv_dtl[i].JobinvoiceTimestamp + 
                    "' totalflatPrice='" + resultData.inv_dtl[i].FlatQty + 
                    "' selectedEmployees='" + JSON.stringify(resultData.inv_dtl[i].Employee) +
                    "'><td>" + (i + 1) + "</td><td work_id='" + resultData.inv_dtl[i].JobCode + 
                    "'>" + resultData.inv_dtl[i].jobtype_name + 
                    "</td><td>" + 
                    resultData.inv_dtl[i].JobDescription + 
                    "</td><td>" + 
                    accounting.formatNumber(resultData.inv_dtl[i].JobQty,3) + 
                    "</td><td>" + 
                    accounting.formatNumber(resultData.inv_dtl[i].JobPrice) + 
                    "</td><td>" + 
                    accounting.formatNumber(resultData.inv_dtl[i].JobDisPercentage) + 
                    "</td><td>" +
                     accounting.formatNumber(resultData.inv_dtl[i].	EstimateSellingPrice) + 
                     "</td><td>" +
                      accounting.formatNumber(resultData.inv_dtl[i].JobNetAmount) + 
              
                    "</td><td>" + actionBtns + "</td></tr>")                
}
    ...
    function calculateProductWiseDiscount(totalNet3, discount, discount_type, disPercent, disAmount, total_amount4) {
        ...
        totalProWiseDiscount = product_discount;
        total_discount = totalProWiseDiscount + total_discount;
    }
    ...
    function addPayment(pcash, pcredit, pcard,pcheque,pcusType,padvance,pbank,preturn) {
        ...
        console.log('pcash',pcash);
        console.log('pcredit',pcredit);
        console.log('pcard',pcard);
        console.log('pcheque',pcheque);
        console.log('pcusType',pcusType);
        console.log('padvance',padvance);
        console.log('pbank',pbank);
        console.log('preturn',preturn);
        console.log('totalNet',totalNet);

        if(pcredit == totalNet ){
            $("#ccard_amount").prop("readonly", true);
            $("#cheque_amount").prop("readonly", true);
            $("#bank_amount").prop("readonly", true);
          
        }else if(pcard == totalNet ){
            $("#credit_amount").prop("readonly", true);
            $("#cheque_amount").prop("readonly", true);
            $("#bank_amount").prop("readonly", true);
        }else if(pcheque == totalNet){
            $("#credit_amount").prop("readonly", true);
            $("#ccard_amount").prop("readonly", true);
            $("#bank_amount").prop("readonly", true);
        }else if (pbank == totalNet){
            $("#credit_amount").prop("readonly", true);
            $("#ccard_amount").prop("readonly", true);
            $("#cheque_amount").prop("readonly", true);
        }else if(pcredit+pcard == totalNet){
            $("#cheque_amount").prop("readonly", true);
            $("#bank_amount").prop("readonly", true);
        }else if(pcredit+pcheque == totalNet){
            $("#ccard_amount").prop("readonly", true);
            $("#bank_amount").prop("readonly", true);
        }else if(pbank+pcredit == totalNet){
            $("#cheque_amount").prop("readonly", true);
            $("#ccard_amount").prop("readonly", true);   
        }else if(pbank+pcard == totalNet){
            $("#credit_amount").prop("readonly", true);
            $("#cheque_amount").prop("readonly", true);
            
        }else if(pbank+pcheque == totalNet){
            $("#credit_amount").prop("readonly", true);
            $("#ccard_amount").prop("readonly", true);
            
        }else if(pcard+pcheque == totalNet){
            $("#credit_amount").prop("readonly", true);
            $("#bank_amount").prop("readonly", true);
        }else if(pcash>totalNet){
            $("#ccard_amount").prop("readonly", true);
            $("#cheque_amount").prop("readonly", true);
            $("#bank_amount").prop("readonly", true);
            $("#credit_amount").prop("readonly", true);
            ...
            $("#ccard_amount").prop("readonly", false);
            $("#cheque_amount").prop("readonly", false);
            $("#bank_amount").prop("readonly", false);
        ...
        // if (pcash > totalNet) {
        //     $.notify("Cash payment cannot exceed total amount.", "error");
        //     $("#btnSave").attr('disabled', true);
        //     return false;
        // }
        // if (pcredit > totalNet) {
        //     $.notify("Credit amount cannot exceed total amount.", "error");
        //     $("#btnSave").attr('disabled', true);
        //     return false;
        // }
        // if (pcard > totalNet) {
        //     $.notify("Card payment cannot exceed total amount.", "error");
        //     $("#btnSave").attr('disabled', true);
        //     return false;
        // }
        // if (pcheque > totalNet) {
        //     $.notify("Cheque amount cannot exceed total amount.", "error");
        //     $("#btnSave").attr('disabled', true);
        //     return false;
        // }
        // if (pbank > totalNet) {
        //     $.notify("Bank payment cannot exceed total amount.", "error");
        //     $("#btnSave").attr('disabled', true);
        //     return false;
        // }
        // if (padvance > totalNet) {
        //     $.notify("Advance amount cannot exceed total amount.", "error");
        //     $("#btnSave").attr('disabled', true);
        //     return false;
        // }
        // if (preturn > totalNet) {
        //     $.notify("Return amount cannot exceed total amount.", "error");
        //     $("#btnSave").attr('disabled', true);
        //     return false;
        // }
        ...
        // if (cusTotal > totalNet) {
        //     $.notify("Total payment cannot exceed the invoice amount.", "error");
        //     $("#btnSave").attr('disabled', true);
        //     return false;
        // }

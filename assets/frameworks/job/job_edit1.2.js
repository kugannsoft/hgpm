                ...
                success: function(data)
                {
                    ...
                    if($.inArray(resultData.job_data.JCusType, companyShowArr )>-1){
                     ...
                     for (var i =  0; i < resultData.job_desc.length; i++) {
                        ...
                        jobAmArr.push(resultData.job_desc[i].JobAmount);
...
loadInvoicetoPrint(jobNo);
    ...
    $("#btnAdd").click(function () {
        ...
        if (jobdesc) {
            jobArr.push(jobdesc);
            jobAmArr.push(jobamount);
            jobNumArr.push(val2); // Assign a unique ID
            ...
            $("#tbl_payment tbody").append(
                `<tr job='${jobdesc}' jobid='${val2}' jobcat='${jobcat}' jobcategory='${jobcategory} jobamount='${jobamount}'>
                    <td>${k}</td>
                    <td></td>
                    <td>${jobdesc}</td>
                    <td>${jobamount}</td>  <!-- Class added for easy reference -->
                    <td><span class='remove btn btn-danger btn-xs'>Remove</span></td>
                </tr>`
            );
    
            k++;
            val2++; // Increment ID to ensure uniqueness
            $("#jobdesc").val('');
            $("#jobdesc2").val('');
            $("#jobamount").val('');
        }
    ...
    $("#tbl_payment tbody").on("click", ".remove", function () {
        ...
        var removeJob = row.attr("job");
        var removeJobId = row.attr("jobid");
        var removeCat = row.attr("jobcat");
        var removeAmount = row.find("td:nth-child(4)").text().trim();
    
        // Remove from arrays
        var jobIndex = jobArr.indexOf(removeJob);
        if (jobIndex !== -1) jobArr.splice(jobIndex, 1);
    
        var numIndex = jobNumArr.indexOf(removeJobId);
        if (numIndex !== -1) jobNumArr.splice(numIndex, 1);
    
        var catIndex = jobcatArr.indexOf(removeCat);
        if (catIndex !== -1) jobcatArr.splice(catIndex, 1);
    
      
        var amIndex = jobAmArr.indexOf(removeAmount);
        if (amIndex !== -1) jobAmArr.splice(amIndex, 1);

...
//     });
    ...
    //                    $("#payAmount").val(0);
                        ...
                        $.ajax({
                            ...
                            data: {receiptType:receiptType,advance_payment_no:advance_payment_no,return_payment_no:return_payment_no,bank_acc:bank_acc,paymentNo: paymentNo, location: location, remark: remark,
                                ...
                                over_pay_inv: over_pay_inv,cardType:cardType},
                            success: function(data)
                            {
                                var resultData = JSON.parse(data);
    ...
    function invoicePrint(invNumber,invoicedate,invfname,total,disPrint,payType,chq_date,chq_no,bank_name,receipType,remarks,paymenttype){
        ...
        if (disPrint != 1) {
            // var invlink = "http://localhost/hgpm.nsoftsoft.com/admin/payment/view_customer_receipt_pdf/"+Base64.encode(invNumber);
            // console.log(invlink);
            ...
            // Open in a new tab for auto print
            window.open(pdfUrl, '_blank');
        
            // Optional: Update UI with last invoice link
            $("#lastInvoice").html("<a href='" + pdfUrl + "' target='_blank'>Last Invoice - " + invNumber + "</a>");
            $("#btnPrint").attr('link', pdfUrl);
        }
    }

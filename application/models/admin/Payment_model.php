...
// WHERE C.`CusCode` = '$cusCode' AND C.`IsCloseInvoice` =0 AND C.`IsCancel` =0 AND C.`Type` != 2  ");
   ...
   public function getActiveCusPayment($table, $q,$location,$cusCode) {
    ...
    public function getReturnAmountById($table, $q,$cusCode) {
        $this->db->select('PRN_No,PRN_Cost_Amount');
        $this->db->like('PRN_No', $q)->where('PRN_IsCancel', 0)->where('PRN_SupCode', $cusCode)->where('IsReturn', 0)->order_by('PRN_No', 'DESC');
        
        $query = $this->db->get($table);
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $new_row['label'] = htmlentities(stripslashes($row['PRN_No']));
                $new_row['value'] = htmlentities(stripslashes($row['PRN_Cost_Amount']));
                $row_set[] = $new_row; //build an array
            }
    ...
    public function loadsuppliersjson($query) {
        ...
        $query1 =$this->db->select('SupCode,SupName')->like("CONCAT(' ',supplier.SupCode,supplier.SupName,supplier.MobileNo)", $query ,'left')
        ->where('IsActive',1)->limit(50)->get('supplier');
    ...
    public function supplierPayment($cpHed,$cpDtl,$post,$paymentNo,$chqDtl) {
        ...
        $this->db->set('SupOustandingAmount', $avail_outstand);
        $this->db->where('SupCode', $cusCode); 
        $this->db->update('supplieroustanding');
        //update customer payment

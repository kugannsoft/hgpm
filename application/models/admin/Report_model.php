...
class Report_model extends CI_Model {
    ...
    public function gensalesreportbyroute($startdate, $enddate, $location = NULL,$locationAr = NULL,$invtype,$salesperson= NULL) {
        if (isset($location) && $location != '') {
            ...
            $this->db->select('customer.MobileNo,customer.CusCode,DATE(salesinvoicehed.SalesDate) As InvDate,(salesinvoicehed.SalesDisAmount) AS DisAmount,salesinvoicehed.SalesInvNo,
                                ...
                                (salesinvoicehed.SalesCustomerPayment) AS CustomerPayment,
                                (salesinvoicehed.SalesCostAmount) AS CostAmount,
                                 (salesinvoicehed.SalesBankAmount) AS BankAmount,
                                (salesinvoicehed.SalesReturnAmount) AS ReturnAmount,customer.CusName,
                                customer.RespectSign,creditinvoicedetails.SettledAmount');
            $this->db->from('salesinvoicehed');
            $this->db->join('customer', 'customer.CusCode = salesinvoicehed.SalesCustomer', 'INNER');
    ...
    public function genjobsumreportbyroute($startdate, $enddate, $location = NULL,$locationAr = NULL) {
        if (isset($location) && $location != '') {
            $this->db->select('DATE(JobInvoiceDate) As InvDate,JobInvNo,(JobTotalDiscount) AS DisAmount,
                                ...
                                (JobNetAmount) AS NetAmount,
                                (JobVatAmount) AS VatAmount,
                                (JobNbtAmount) AS NbtAmount,JRegNo,JobCardNo,JobBankAmount');
            $this->db->from('jobinvoicehed');
            $this->db->where('DATE(JobInvoiceDate) <=', $enddate);
            $this->db->where('DATE(JobInvoiceDate) >=', $startdate);
            $this->db->where_in('JobLocation', $locationAr);
    ...
    public function loadjobcardsummery($startdate, $enddate, $location = NULL,$locationAr = NULL) {
        if (isset($location) && $location != '') {
            ...
            $this->db->join('model', 'model.model_id = vehicledetail.Model', 'INNER');
    ...
    public function loademployeeproductivity($startdate, $enddate, $emp) {
    ...
    public function salesinvoicedaysalesumreportbypayment($startdate, $enddate, $location = NULL,$locationAr = NULL) {
        if (isset($location) && $location != '') {
            $this->db->select('salesinvoicehed.*,salesinvoicedtl.*');
            $this->db->from('salesinvoicehed');
            $this->db->join('salesinvoicedtl', 'salesinvoicedtl.SalesInvNo = salesinvoicehed.SalesInvNo', 'INNER');
    ...
    public function genreportbyproduct($startdate, $enddate, $location = NULL, $product = NULL,$locationAr = NULL,$dep,$subdep,$subcat) {
        $this->db->select('salesinvoicedtl.SalesProductCode,
                          ...
                          (salesinvoicedtl.SalesQty) AS Qty,
                           (salesinvoicedtl.SalesFreeQty) AS FreeQty,
                          (salesinvoicedtl.SalesCostPrice * salesinvoicedtl.SalesQty) AS CostValue,
                           ...
                           (salesinvoicedtl.SalesReturnQty) AS ReturnQty,
...
//        $this->db->group_by('invoicedtl.InvNo');
    ...
    public function gencashreportbypart($startdate, $enddate, $location = NULL, $locationAr = NULL) {
        $this->db->select('SalesProductCode,
                          ...
                          (SalesCostPrice * SalesQty) AS CostValue,
                           ...
                           (salesinvoicehed.SalesDisAmount) AS TotalDiscount,salesinvoicehed.InvIsCancel,
                (salesinvoicedtl.SalesSerialNo) AS Serial');
        $this->db->from('salesinvoicedtl');
        $this->db->join('salesinvoicehed', 'salesinvoicehed.SalesInvNo = salesinvoicedtl.SalesInvNo', 'INNER');
    ...
    public function genreporttotalDiscountbyproduct($startdate, $enddate, $location = NULL, $product = NULL,$locationAr = NULL,$dep,$subdep,$subcat) {
        $this->db->select('SUM(salesinvoicehed.SalesDisAmount) AS TotalDiscount');
        $this->db->from('salesinvoicehed');
        $this->db->join('salesinvoicedtl', 'salesinvoicehed.SalesInvNo = salesinvoicedtl.SalesInvNo', 'INNER');
        $this->db->join('product', 'product.ProductCode = salesinvoicedtl.SalesProductCode', 'INNER');
        $this->db->join('department', 'department.DepCode = product.DepCode', 'left');
        $this->db->where('DATE(salesinvoicehed.SalesDate) <=', $enddate);
        $this->db->where('DATE(salesinvoicehed.SalesDate) >=', $startdate);
        ...
        $this->db->group_by('salesinvoicedtl.SalesInvNo');
        //$this->db->group_by('ProductCode');
        // $this->db->limit(50);
        $result=$this->db->get();
        $list=0;
    ...
    public function searchsalesperson($q) {
        $this->db->select('RepID AS id,RepName AS text');
        $this->db->from('salespersons');
        $this->db->like('CONCAT(RepID,RepName)', $q, 'left');
        $this->db->limit(50);
        return $this->db->get()->result();
    }
...
//        return $result;
    ...
    public function loadsuppliercreditoutstanding($startdate, $enddate, $loc = NULL, $supCode) {
        $this->db->select('creditgrndetails.*');
        $this->db->from('creditgrndetails');
    ...
    public function loadsuppliercredisummury($startdate, $enddate, $loc = NULL, $supCode) {
    ...
    public function loadsupplier($startdate, $enddate, $loc = NULL, $supCode) {
        $this->db->select('supplier.SupName,supplier.MobileNo');
        $this->db->from('supplier');
        $this->db->where('SupCode',$supCode);
        return $this->db->get()->result();
    }
...
//                $this->db->where('creditinvoicedetails.CreditAmount !=creditinvoicedetails.SettledAmount');
    ...
    public function genemployeewisejobsale($startdate, $enddate, $location = NULL,$locationAr = NULL) {
        if (isset($location) && $location != '') {
            $this->db->select('DATE(jobinvoicehed.JobInvoiceDate) As InvDate,jobinvoicehed.JobInvNo,SUM(jobinvoicehed.JobTotalDiscount) AS DisAmount,
                                SUM(jobinvoicehed.JobCashAmount)+ SUM(jobinvoicehed.ThirdCashAmount) AS CashAmount,
                                SUM(jobinvoicehed.JobCardAmount)+ SUM(jobinvoicehed.ThirdCardAmount) AS CCardAmount,
                                SUM(jobinvoicehed.JobCreditAmount)+ SUM(jobinvoicehed.ThirdCreditAmount) AS CreditAmount,
                                SUM(jobinvoicehed.JobChequeAmount)+ SUM(jobinvoicehed.ThirdChequeAmount) As ChequeAmount,
                                SUM(jobinvoicehed.JobCompanyAmount) AS CompanyAmount,
                                SUM(jobinvoicehed.JobTotalAmount) AS InvAmount,
                                SUM(jobinvoicehed.JobAdvance) AS AdvanceAmount,
                                SUM(jobinvoicehed.JobNetAmount) AS NetAmount,
                                SUM(jobinvoicehed.JobVatAmount) AS VatAmount,
                                SUM(jobinvoicehed.JobNbtAmount) AS NbtAmount,jobinvoicehed.JRegNo,jobcardhed.Jlabour,salespersons.RepName');
            $this->db->from('jobinvoicehed');
            $this->db->join('jobcardhed','jobinvoicehed.JobCardNo = jobcardhed.JobCardNo','INNER');
            $this->db->join('salespersons','jobcardhed.Jlabour = salespersons.RepID','LEFT');
            $this->db->where('DATE(jobinvoicehed.JobInvoiceDate) <=', $enddate);
            $this->db->where('DATE(jobinvoicehed.JobInvoiceDate) >=', $startdate);
            $this->db->where_in('jobinvoicehed.JobLocation', $locationAr);
            $this->db->where('jobinvoicehed.IsCancel', 0);
            $this->db->group_by('jobcardhed.Jlabour');
            $this->db->order_by('jobinvoicehed.JobInvoiceDate', 'DESC');
            // $this->db->limit(50);
        } else {
            $this->db->select('DATE(jobinvoicehed.JobInvoiceDate) As InvDate,jobinvoicehed.JobInvNo,SUM(jobinvoicehed.JobTotalDiscount) AS DisAmount,
                                SUM(jobinvoicehed.JobCashAmount)+ SUM(jobinvoicehed.ThirdCashAmount) AS CashAmount,
    ...
    public function genjobreportbyservicesbyjobtype($startdate, $enddate, $location = NULL, $product = NULL,$locationAr = NULL,$dep,$subdep,$subcat) {
        $this->db->select('jobinvoicedtl.JobDescription AS AppearName,
                           jobinvoicedtl.JobCost,
                           jobinvoicedtl.JobPrice,
                          SUM(jobinvoicedtl.JobQty) AS Qty,
                          SUM(jobinvoicedtl.JobCost * jobinvoicedtl.JobQty) AS CostValue,
                           SUM(jobinvoicedtl.JobTotalAmount) AS TotalAmount,
                           SUM(jobinvoicedtl.JobDiscount) AS DisAmount,
                           SUM(jobinvoicedtl.JobVatAmount) AS VatAmount,
                           SUM(jobinvoicedtl.JobNbtAmount) AS NbtAmount,
                           SUM(jobinvoicedtl.JobNetAmount) AS NetAmount,
                           (jobinvoicehed.JobTotalDiscount) AS TotalDiscount,salespersons.RepName');
        $this->db->from('jobinvoicedtl');
        $this->db->join('jobcardhed','jobinvoicedtl.JobCardNo = jobcardhed.JobCardNo','INNER');
        $this->db->join('salespersons','jobcardhed.Jlabour = salespersons.RepID','LEFT');
        $this->db->join('jobinvoicehed','jobinvoicedtl.JobInvNo = jobinvoicehed.JobInvNo','INNER');
//        $this->db->join('jobtype', 'jobtype.jobtype_id = jobinvoicedtl.JobType', 'INNER');
        ...
        $this->db->where('jobinvoicehed.IsCancel', 0);
        $this->db->where_not_in('jobinvoicedtl.JobType', array('9','3'));
        if (isset($location) && $location != '') {
            $this->db->where_in('jobinvoicehed.JobLocation', $locationAr);
        }
        if (isset($product) && $product != '') {
        ...
        // $this->db->group_by('jobinvoicedtl.jobtype');
        $this->db->group_by('jobcardhed.Jlabour');
//        $this->db->order_by('jobinvoicedtl.JobCardNo');
//        $this->db->order_by('jobtype.jobtype_order', 'ASC');

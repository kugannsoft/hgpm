...
class Grn_model extends CI_Model {
    ...
    public function loadsupplierjson($query) {
        ...
        $query1 =$this->db->select('SupCode,SupName')
        ->like("CONCAT(' ',supplier.SupCode,supplier.SupName,supplier.MobileNo)", $query ,'left')
        ->where('IsActive',1)
        ->get('supplier');
    ...
    public function loadproductjsonforgrn($query,$sup,$supCode) {
        if($sup!=0){
    ...
    public function getSupplierCreditLimit($query) {
        $q= $this->db->select('CreditLimit')->from('supplier')->where('SupCode',$query)->get()->result();
        return json_encode($q);
    }
...
//        $this->db->trans_begin();
        $this->db->trans_start();
            ...
            for ($i = 0; $i < count($product_codeArr); $i++) {
                ...
                if ($productData && $stockData) {
                    $oldQty = $stockData->Stock; 
                    $oldCost = $productData->Prd_AveragecostPrice; 
                    $newQty = $totalGrnQty;
                    $newCost = $cost_priceArr[$i];
                   ...
                   $this->db->update(
                    ...
                    // $this->db->update('productstock', array('Stock' => $updatedStockQty), array('ProductCode' => $product_codeArr[$i], 'Location' => $location));
                }
    ...
    public function loadGrnById($inv) {
        return $this->db->select('goodsreceivenotedtl.*,product.Prd_Description,goodsreceivenotehed.GRN_Amount AS GRN_totalAmount,goodsreceivenotehed.GRN_NetAmount AS totalNet, 
        goodsreceivenotehed.GRN_DisAmount As TotalDiscount,goodsreceivenotehed.GRN_SupCode')->from('goodsreceivenotedtl')

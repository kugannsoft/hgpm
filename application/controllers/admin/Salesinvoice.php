...
class Salesinvoice extends Admin_Controller {
    ...
    public function view_invoice($inv=null) {
            ...
            $this->data['invHed']=  $this->db->select('JobCardNo AS jobcardNo,JCustomer AS customerCode,JRegNo AS regNo,JobInvoiceDate AS date,
            JobAdvance,JobTotalDiscount,JobNetAmount,JobVatAmount,JobNbtAmount,JobTotalAmount,JobBankAmount,JobCashAmount,JobCreditAmount,JobChequeAmount,JobCardAmount,EstType,JJobType,IsPayment,JobEstimateNo,
            JobSupplimentry,vehicle_company.VComName AS JInsCompany,JobInvNo,IsCancel,InvoiceType,users.first_name,users.last_name,TempNo,VComName,InvRemark,mileageout,mileageoutUnit,JobCreditAmount,jobinvoicehed.InvRemark')
                ->from('jobinvoicehed')
                ->join('users','jobinvoicehed.JobInvUser=users.id','left')
                ->join('vehicle_company','jobinvoicehed.JInsCompany=vehicle_company.VComId','left')
    ...
    public function cancel_tem_invoice()
    {
     ...
     public function view_temp_invoice($inv=null) {
            ...
            $this->data['last_job'] = $this->db->select('max(date(jobcardhed.appoimnetDate))  AS lastjob')->from('jobcardhed')->where('jobcardhed.JRegNo', $regNo)->where('jobcardhed.JCustomer', $cusCode)->where('jobcardhed.IsCancel', 0)->where('jobcardhed.appoimnetDate<', $appoimnetDate)->get()->row()->lastjob;
             ...
             $this->data['term'] = $this->db->select()->from('invoice_condition')->where('InvType', 2)->get()->result();
            $result=$this->db->select('tempjobinvoicedtl.*,jobtype.jobtype_name AS SubDescription , jobtypeheader.jobhead_name AS Description')
            ->from('tempjobinvoicedtl')
            ->join('jobtype', 'tempjobinvoicedtl.JobType = jobtype.jobtype_id', 'INNER')
            ->join('jobtypeheader', 'jobtypeheader.jobhead_id = jobtype.jobhead', 'INNER')
            ->where('tempjobinvoicedtl.JobInvNo', $invNo)
            ->order_by('jobtypeheader.jobhead_order', 'ASC')
            ->order_by('jobtype.jobtype_order', 'ASC')
            ->order_by('tempjobinvoicedtl.JobinvoiceTimestamp','ASC')
            ->order_by('tempjobinvoicedtl.EstLineNo', 'ASC')->get();
            $list = array();
            foreach ($result->result() as $row) {
                $list[$row->Description][] = $row;
            }
    ...
    public function getInvoiceDataByInvoiceNo(){
        ...
        if($invoiceNo!='' && isset($invoiceNo)){
            ...
            if($isInvoice>0){
                ...
                $arr['inv_dtl'] = $this->db->select('jobinvoicedtl.*,jobtype.jobtype_name')->from('jobinvoicedtl')
                ->join('jobtype', 'jobtype.jobtype_id = jobinvoicedtl.JobType')
                ->where('jobinvoicedtl.JobInvNo', $invoiceNo)
                ->order_by('jobinvoicedtlid')->get()->result();
...
$arr[] =null;
    ...
    public function saveTempInvoices() {
        ...
        if($action==1){
            ...
            $this->db->where('JobCardNo', $data['JobCardNo']);
            $this->db->update('jobcardhed', ['is_invoice' => 1]);
            ...
            for ($i = 0; $i < count($work_idArr); $i++) {
                ...
                foreach ($employeeList as $employee) {
                    ...
                    $this->db->insert('jobflat_details', $jobFlatDtl);
                }
        ...
        $this->db->trans_start();
            ...
            $this->db->where('Job_Inv_No', $data['JobInvNo'])->delete('jobflat_details');
            $estTimestmp = '';
            $estLineNo =0;
...
//update payment details
        ...
        if($bankAmount>0){
         ...
         if($chequeAmount>0){
            ...
            // $flatQtyArr = json_decode($_POST['totalflatPrice']); 
            // $EmployeeArr = json_decode($_POST['selectedEmployeesArr']); 
            // $selectedEmployeesArr = json_decode($_POST['selectedEmployeesArr'][0], true);
               ...
               $estTimestmp = '';
                ...
                for ($i = 0; $i < count($work_idArr); $i++) {
                    ...
                    if($job_idArr[$i]==2){
                        ...
                        // $flatQty = isset($flatQtyArr[$i]) ? $flatQtyArr[$i] : 0; 
                        // $employeeData = !empty($selectedEmployeesArr) ? json_encode($selectedEmployeesArr) : json_encode([]);
                         $partDtl = array(
                            'AppNo' => 1,
                ...
                //         'FlatQty' => $flatQty,
                //         'CostPrice' => $costPriceArr[$i],
                //         'ProfitAmount' => ($costPriceArr[$i]* $qtyArr[$i]) - ($costPriceArr[$i] * $flatQtyArr[$i]),
                //         'Date' => date("Y-m-d H:i:s")
                //     );

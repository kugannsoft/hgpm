...
class Job extends Admin_Controller {
    ...
    public function index() {
        ...
        if (!$this->ion_auth->logged_in() OR !$this->ion_auth->is_admin()) {
            ...
            $this->data['joblocation'] = $this->db->select()->from('job_location')->get()->result();
            $this->data['labour'] = $this->db->select()->from('salespersons')->where('IsActive', 1)->get()->result();
    ...
    public function do_upload(){
        ...
        if ( ! $this->upload->do_upload()){
            ...
            $job_data = array('job_no' => $jobno,'doc_name' => $doc_name,'upload_by' => $invuser,'upload_date' => $invDate,'ext' => $ext );
    ...
    public function view_estimate($estNo=null) {
        ...
        if (!$this->ion_auth->logged_in() OR !$this->ion_auth->is_admin()) {
            ...
            $this->data['invCus']= $this->db->select('customer.*')
                ->from('customer')->join('vehicledetail','vehicledetail.CusCode=customer.CusCode')->where('customer.CusCode',$cusCode)->get()->row();
            $this->data['invVehi']= $this->db->select('vehicledetail.ChassisNo,vehicledetail.contactName,make.make,model.model,vehicledetail.Color AS body_color,fuel_type.fuel_type,vehicledetail.Milage')
            ->from('vehicledetail')->join('make','make.make_id=vehicledetail.Make','left')
    ...
    public function all_estimate_customer() {
        if (!$this->ion_auth->logged_in() OR !$this->ion_auth->is_admin()) {
            redirect('auth/login', 'refresh');
        } else {
            /* Title Page */
            ...
            /* Load Template */
            $this->template->admin_render('admin/job/all_estimate_customer', $this->data);
        }
    }
...
//            $this->data['company'] = $this->Job_model->get_data_by_where('company', $id3);
    ...
    public function loadJobDescJsonByType(){
        $query = $_GET['q'];
        $type = $_GET['workType'];
        $q = $this->db->select('JobDescNo AS id,JobDescription AS text,JobCost AS jobCost,isVat,isNbt,nbtRatio,FlatQty')->from('inv_jobdescription')->where('jobtype', $type)->like('JobDescription', $query)->get()->result();
        echo json_encode($q);die;
    }
    ...
    public function getVehicleDetailsByIdandCus() {
       ...
       $data= $this->db->select('*,vehicledetail.Color AS body_color,customer.MobileNo')
       ->from('vehicledetail')
       ->where('vehicledetail.RegNo', $cat)->where('vehicledetail.CusCode', $cus)
       ->join('make', 'make.make_id = vehicledetail.Make', 'left')
       ->join('fuel_type', 'fuel_type.fuel_typeid = vehicledetail.FuelType', 'left')
       ->join('model', 'model.model_id = vehicledetail.Model', 'left')
       ->join('customer', 'customer.CusCode = vehicledetail.CusCode', 'left')
       ->get()->row();
        echo json_encode($data);die;
    }
    ...
    public function allestimates() {
        ...
        if ($checkRole == 6) {
            $this->datatables->where('estimatehed.EstCustomer', $cusCode);
        }
    ...
    public function saveJob() {
        ...
        $data['Jlabour']=$_POST['labour'];
        $data['JobLocation']=$_POST['jobLocation'];

        $jobtypeArr = $_POST['jobSection'];

        
        $jobArr = json_decode($_POST['jobArr']);
        $jobAmArr = json_decode($_POST['jobAmArr']);
...
redirect('admin/job/estimate_job/'.Base64_encode($EstimateNo). 'refresh');
    ...
    public function view_job_customer() {
        if (!$this->ion_auth->logged_in() OR !$this->ion_auth->is_admin()) {
            redirect('auth/login', 'refresh');
        } else {
            /* Title Page */
            ...
            /* Load Template */
            $this->template->admin_render('admin/job/view-job-customer', $this->data);
        }
    }
}

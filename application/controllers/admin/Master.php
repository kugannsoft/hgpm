...
//        $this->lang->load('admin/master');
    ...
    public function addColor() {
        $data['body_color'] = $_POST['department'];
        $data['result'] = $this->db->insert('body_color', $data);
        $data['bodycolor_id'] = $this->db->insert_id();
        if($data['result']) {
            echo json_encode($data);
        }
        die;
    ...
    public function addmanYear() {
        $data['year'] = $_POST['department'];
        $data['result'] = $this->db->insert('year', $data);
        $data['year_id'] = $this->db->insert_id();
        if($data['result']) {
            echo json_encode($data);
        }
        die;
    ...
    public function addDom() {
        $data['domastric'] = $_POST['department'];
        $data['result'] = $this->db->insert('domastric', $data);
        $data['dom_id'] = $this->db->insert_id();
        if($data['result']) {
            echo json_encode($data);
        }
        die;

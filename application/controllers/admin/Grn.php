...
class Grn extends Admin_Controller {
    ...
    public function addgrn() {
        ...
        $role_id = $this->db->select('role')->from('users')->get()->row()->role;
        
        $permission = $this->db->select('per_class,is_add')->from('system_permission_set')
        ->where('role_id',$role_id)
        ->where('per_class','Good Received Note')
        ->where('is_add',1)->get()->row();
       
        
        if ( (!empty($permission) && $permission->is_add == 1)) {
    ...
    public function getSupplierCreditLimit() {
        $query = $_REQUEST['supplierCode'];
        // echo var_dump($query);die;
        echo $this->Grn_model->getSupplierCreditLimit($query);
        die;
    }
    ...
    public function loadselling() {
        $query = $_GET['q'];
        $sup= $_REQUEST['sup'];$supCode= $_REQUEST['supcode'];
        echo $this->Grn_model->loadselling($query,$sup,$supCode);
        die;
    }

    public function loadproductjsonforgrn() {
        $query = $_GET['q'];
        $sup= $_REQUEST['sup'];$supCode= $_REQUEST['supcode'];
        echo $this->Grn_model->loadproductjsonforgrn($query,$sup,$supCode);
        die;
    }
    ...
    public function saveGrn() {
        ...
        $total_vat_net_amount = $_POST['total_vat_net_amount'];
        ...
        $pro_vatArr = json_decode($_POST['sendProVat']);
        ...
        $grnHed = array(
            ...
            'GRN_Amount' => $total_amount,'GRN_NetAmount' => $total_net_amount,'GRN_VatNetAmount' => $total_vat_net_amount,'GRN_DueAmount' => 0,'GRN_ReturnAmount' => 0,
...
//            $company = 'VCOM';
    ...
    public function getActiveGRN() {
        if (isset($_GET['name_startsWith'])) {
            ...
            $this->db->select('goodsreceivenotehed.GRN_No,creditgrndetails.SettledAmount');
            $this->db->from('goodsreceivenotehed');
            $this->db->join('creditgrndetails', 'creditgrndetails.GRNNo = goodsreceivenotehed.GRN_No', 'inner');
            ...
            $this->db->where('creditgrndetails.SettledAmount','0.00');
       
    
    
            $this->db->order_by('goodsreceivenotehed.GRN_No', 'DESC');
            $this->db->group_by('goodsreceivenotehed.GRN_No');
            ...
            if ($query->num_rows() > 0) {
                foreach ($query->result_array() as $row) {
                    $row_set[] = [
                        'label' => htmlentities($row['GRN_No']),
                        'value' => htmlentities($row['GRN_No'])
                    ];
                }
            }

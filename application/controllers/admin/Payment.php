...
/*shalika*/
    ...
    public function view_customer($cuscode=null) {
        ...
        $this->data['credit_inv']=$this->db->select('*, SUM(returnAmount) AS returnAmounts')->from('creditinvoicedetails')->where('CusCode',$cuscode)->where('IsCancel',0)->group_by('InvoiceNo')->get()->result();
...
//          $this->data['return_payment']= $return;
        ...
        if ($checkRole == 6) {
            $this->template->admin_render('admin/payment/view_customer_wise', $this->data);
        } else {
...
//            }
    ...
    public function view_supplier($supcode=null) {
        ...
        $this->data['credit_inv']=$this->db->select()->from('creditgrndetails')->where('SupCode',$supcode)->where('IsCloseGRN',0)->where('IsCancel',0)->get()->result();
    ...
    public function customerPayment() {
        ...
        $cpHed = array(
            ...
            'ChequePay' => $cheque_amount, 'Remark' => $remark, 'CardPay' => $card_amount, 'BankPay' => $bank_amount,'TotalPayment' => $total_settle, 'AvailableOustanding' => $avail_outstand, 'CancelUser' => 0, 'SystemUser' => $invUser, 'IsCancel' => 0
        );
 ...
 /*=========Cancel Customer payment===========================================*/
    ...
    public function getSuppliersDataById() {
        ...
        $arr['credit_data'] =$this->db->select('*,goodsreceivenotehed.GRN_InvoiceNo AS invNo,return_grn_details.ReturnAmount')
                        ...
                        ->get()->result();
        $arr['return_data'] = $this->db->select('SUM(PRN_Cost_Amount) AS Total')->from('purchasereturnnotehed')
                            ->where('purchasereturnnotehed.PRN_SupCode', $supCode)
                            ->where('purchasereturnnotehed.IsReturn',0)
                            ->get()->row();
        echo json_encode($arr);
        die;
    }
    ...
    public function getReturnAmountById() {
         
        if (isset($_GET['name_startsWith'])) {
            $q = strtolower($_GET['name_startsWith']);
        
            $cus = $_GET['cus_code'];
            $this->Payment_model->getReturnAmountById('purchasereturnnotehed', $q,$cus);
            die;
        }
    }
    ...
    public function supplierPayment() {
        ...
        if($payType ==4){
            if($cashType ==2){
                ...
                // echo var_dump($cus_settle_amount);die;
                // $result = [];
                $this->db->select('SupOustandingAmount');
                $this->db->from('supplieroustanding');
                $this->db->where('SupCode', $cusCode);
                $outstandingQuery = $this->db->get();

                $this->db->select('IsReturn');
                $this->db->from('purchasereturnnotehed');
                $this->db->where('PRN_No',$returnInvoice);
                $returnQuery = $this->db->get();

                $updateIsReturn = [
                    'IsReturn' => 1
                ];
                $this->db->where('PRN_No',$returnInvoice);
                $this->db->update('purchasereturnnotehed', $updateIsReturn);

                $currentSupOustandingAmount = $outstandingQuery->row()->SupOustandingAmount;
                $newcurrentSupOustandingAmount = $currentSupOustandingAmount -  $payAmount;

                $updateData1 = [
                    'SupOustandingAmount' => $newcurrentSupOustandingAmount
                ];
    
                $this->db->where('SupCode', $cusCode);
                $this->db->update('supplieroustanding', $updateData1);

                foreach ($credit_invoice as $key => $grnNo) {
                    if ($cus_settle_amount[$key] > 0) {
                        $this->db->select('CreditAmount');
                        $this->db->from('creditgrndetails');
                        $this->db->where('GrnNo', $grnNo);
                        $query = $this->db->get();
                
                        if ($query->num_rows() > 0) {
                            $currentCreditAmount = $query->row()->CreditAmount;
                            $newCreditAmount = $currentCreditAmount - $cus_settle_amount[$key];
                            ...
                            $this->db->where('GrnNo', $grnNo);
                            $this->db->update('creditgrndetails', $updateData);
                
                            $data = [
                                'GrnNo' => $grnNo,
                                'ReturnAmount' => $cus_settle_amount[$key],
                                'PayDate' => $payDate,
                            ];
                
                            $this->db->insert('return_grn_details', $data);
                        } else {
                            echo "No record found for GRN No " . $grnNo . "<br>";
                        }
                    }
                }
            ...
            if($cashType ==2){
                ...
                $this->Payment_model->bincard($paymentNo,12,'Created');
                $return = array(
                    'InvNo' => $paymentNo,
                    'InvDate' => $invDate,
                    'fb' => $res2,
    ...
    public function cancelSupPayment() {
        ...
        $cancelAmount = $this->db->select('PayAmount')->from('supplierpaymentdtl')->where('SupPayNo', $paymentNo)->get()->row()->PayAmount;
        ...
        $invCanel = [
            ...
            'TransactionCode' => 3,
            'CounterNo' => 1,
            'Remark' => $remark . $paymentNo,
            'FlotAmount' => $cancelAmount,
            'SystemUser' =>1,
        ];
    
            $invCanel['CurrentBlance'] = $currentBalance + $cancelAmount;
            $res3 = $this->Cash_model->saveCashFloat('maincashflot', $invCanel);
        $return = [
            'InvNo' => $paymentNo,
            'CancelNo' => $cancelNo,
...
if($paymonut!=''){
    ...
    public function fixReturnInvoice()
    {
        ...
        $q = $this->db->select('*')
            ...
            ->where('IsCloseInvoice', 1)
            ->where('SettledAmount ', 0.00)
            ->where('Type', 1)
            ->get()->result();

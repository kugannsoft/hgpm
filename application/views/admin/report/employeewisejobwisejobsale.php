...
<script>
    ...
    $('#filterform').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: "loadjobsalebyservicebyjobtype",
            data: $(this).serialize(),
    ...
    function drawTable(data) {
        ...
        for (var i = 0; i < value.length; i++) {
            ...
            if (i == (value.length - 1)) {
                $("#saletable").append("<tr style='background-color:#3c8dbc;color:#fff;'><td colspan='1' >Total</td><td><b></b></td><td class='cos'>" + accounting.formatMoney(gr_qty) + "</td><td></td><td></td><td><b></b></td><td><b></b></td><td><b>" + accounting.formatMoney(gr_net) + "</b></td></tr>");
    ...
    function drawRow(rowData) {
        ...
        cost = parseFloat(rowData.CostValue);
        profit = parseFloat(rowData.NetAmount - cost);

        proDisount+=parseFloat(rowData.DisAmount);
        var date = new Date(rowData.JobInvoiceDate);
        row.append($("<td>" + rowData.RepName + "</td>"));
        row.append($("<td class='invunitpr' align='right'>" + accounting.formatMoney(rowData.JobPrice) + "</td>"));
        row.append($("<td class='qty' align='right'>" + accounting.formatMoney(rowData.Qty) + "</td>"));
        row.append($("<td class='totalamount' align='right'>" + accounting.formatMoney(rowData.TotalAmount) + "</td>"));
        row.append($("<td class='disamount' align='right'>" + accounting.formatMoney(parseFloat(rowData.DisAmount)) + "</td>"));
        row.append($("<td class='vatamount' align='right'>" + accounting.formatMoney(parseFloat(rowData.VatAmount)) + "</td>"));
        row.append($("<td class='nbtamount' align='right'>" + accounting.formatMoney(parseFloat(rowData.NbtAmount)) + "</td>"));
